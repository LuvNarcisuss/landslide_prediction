...
def imp_lightgbm_model(verbose = False):
    ...
    model = LGBMClassifier(**best_params, random_state=42, verbose=-1, class_weight='balanced')
    model.fit(X_train, y_train)
    # 保存模型
    joblib.dump(model, os.path.join(os.path.dirname(os.path.dirname(__file__)), 'models', 'imp_lightgbm.pkl'))
...
if __name__ == "__main__":
    imp_lightgbm_model(verbose = True)
